 ...
 * @brief 初始化ALK8266WIFI模组的片选信号(nCS)和复位信号(nRESET)对应的主机GPIO。
...
void M8266HostIf_GPIO_CS_RESET_Init(void)	//本文件的M8266HostIf_Init()函数(下下方)
{
	...
	M8266WIFI_SPI_nCS_GPIO_RCC_CLOCK_EN;  						// 使能 nCS GPIO clock
	...
	M8266WIFI_nRESET_GPIO_RCC_CLOCK_EN;  						// 使能nRESET GPIO CLOCK
...
/***********************************************************************************
 * @brief 初始化 ALK8266WIFI 模组所连接的主机的 SPI 接口。 You may update the macros of 
 * 			SPI usages for nRESET from brd_cfg.h, 不建议修改
 * @param void
 * @return void
 ***********************************************************************************/
...
//-------------  STM32F1  -----------------------------------------------------------
	...
	RCC_APB2PeriphClockCmd(RCC_APB2Periph_GPIOA, ENABLE); 	// 使能SPI1 GPIO CLOCK
	GPIO_InitStructure.GPIO_Pin		= GPIO_Pin_5|GPIO_Pin_6|GPIO_Pin_7;	// GPIOA的PA5,PA6,PA7
	GPIO_InitStructure.GPIO_Mode 	= GPIO_Mode_AF_PP;		// 为SPI1设置GPIOs模式
	GPIO_InitStructure.GPIO_Speed 	= GPIO_Speed_50MHz;		// 设置GPIO频率为50MHz
...
{
	M8266HostIf_GPIO_CS_RESET_Init();	// SPI_nCS 和 nRESET 管脚对应的 GPIO 初始化
	M8266HostIf_SPI_Init();				// SPI主机接口的初始化
...
//----------------------------------------------------------------------------------
// M8266WiFiDrv.lib 需要以下函数。 请根据您的硬件实施它们
//----------------------------------------------------------------------------------

/***********************************************************************************
 * @brief M8266WIFI主机在 nRESET 所对应的GPIO管脚上输出高低电平。，您可以从 brd_cfg.h 更新nRESET的GPIO PIN使用宏。不建议您修改以下代码
 * @param level 主机在 GPIO 上的输出电平(0 = 输出低电平， 即复位模组。 1/其他 = 输出高电平， 即退出复位。)
 * @return void
 ***********************************************************************************/
...
//
/***********************************************************************************
 * @brief 主机在 SPI_nCS 所对应的 GPIO 管脚上输出高低电平，实现对模组的 SPI 片选控制。(brd_cfg.h中可修改)
 * @param level: 主机在 GPIO 上的输出电平。	(0 = 输出低电平，即片选中模组。其他 = 输出高电平，即不片选模组。)
 * @return void
 ***********************************************************************************/
 ...
 * @brief 向 SPI 总线接口（MOSI）写一个字节，同时，从 SPI 总线接口（MISO）读取回一个字节。(brd_cfg.h中可修改)
 * @param TxdByte: 主机向 SPI 写的字节数值。
 * @return (u8) SPI 总线向主机返回的字节数值。                                                                         *
 ***********************************************************************************/
